...
class IncomeTestCase(TestCase):
    ...
    def test_owner_create_income(self):
        ...
        response = self.client.post(
            f"/budget/{self.budget.pk}/incomes/", data=data, headers={"Authorization": f"Bearer {self.owner_token}"}
        )
    ...
    def test_viewer_create_income(self):
        ...
        response = self.client.post(
            f"/budget/{self.budget.pk}/incomes/", data=data, headers={"Authorization": f"Bearer {self.viewer_token}"}
        )

        self.assertEqual(response.status_code, 404)

    def test_owner_delete_income(self):
        income = Income.objects.create(budget=self.budget, name="gas", amount=Decimal("99.5"), category="car")
        income.save()

        response = self.client.delete(
            f"/budget/{self.budget.pk}/incomes/{income.pk}/", headers={"Authorization": f"Bearer {self.owner_token}"}
        )
    ...
    def test_viewer_delete_income(self):
        ...
        response = self.client.delete(
            f"/budget/{self.budget.pk}/incomes/{income.pk}/", headers={"Authorization": f"Bearer {self.viewer_token}"}
        )
